(() => {
  ...
  async function copyCurrentCode(button){
    ...
    if(!code) return;
    try{ await navigator.clipboard.writeText(code); const old=button.innerHTML; button.textContent='Código copiado ✓'; setTimeout(()=>{button.innerHTML=old;},1600); }
  ...
  async function invitationCenter(){
    const {bg}=modal('Convites','Veja os convites recebidos e acompanhe os que você enviou.','<div class="cv9-tabs"><button class="cv9-tab active" data-cv9-tab="received">Recebidos</button><button class="cv9-tab" data-cv9-tab="sent">Enviados</button></div><div data-cv9-content><div class="cv9-empty">Carregando...</div></div>',true);
