(() => {
  ...
  function bucketChartData() {
    ...
    if(range.period==='today'){const buckets=Array.from({length:24},(_,hour)=>({label:`${String(hour).padStart(2,'0')}h`,correct:0,total:0}));source.forEach(item=>{if(!item.date)return;const bucket=buckets[item.date.getHours()];bucket.correct+=item.correct;bucket.total+=item.total;});return buckets.map(item=>({label:item.label,value:item.total?Math.round(item.correct/item.total*100):0}));}
    if(range.period==='month'){const totalDays=new Date(range.end.getFullYear(),range.end.getMonth()+1,0).getDate();const buckets=Array.from({length:totalDays},(_,index)=>({label:String(index+1),correct:0,total:0}));source.forEach(item=>{if(!item.date)return;const bucket=buckets[item.date.getDate()-1];if(!bucket)return;bucket.correct+=item.correct;bucket.total+=item.total;});return buckets.map(item=>({label:item.label,value:item.total?Math.round(item.correct/item.total*100):0}));}
