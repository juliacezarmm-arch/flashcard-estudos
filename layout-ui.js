...
.home-top-streak .fire img { width: 18px; height: 18px; object-fit: contain; vertical-align: middle; }
  ...
  function openHome() { document.body.classList.add('home-active'); document.querySelectorAll('.view').forEach(view => view.classList.toggle('active', view === homeView)); tabs.querySelectorAll('.tab').forEach(button => { const active = button === homeTab; button.classList.toggle('active', active); active ? button.setAttribute('aria-current', 'page') : button.removeAttribute('aria-current'); }); setHomePanel('today'); renderHome(); if (typeof closeMobileNav === 'function') closeMobileNav(); }
   ...
   function progressDayKey(date) { return localDateKey(date); }
    function progressDayOffset(offset) { const date = new Date(); date.setHours(0, 0, 0, 0); date.setDate(date.getDate() + offset); return date; }
    function progressDateRangeFrom(referenceDate, days = 7) { const end = new Date(referenceDate); end.setHours(0, 0, 0, 0); return Array.from({ length: days }, (_, index) => { const date = new Date(end); date.setDate(end.getDate() + index - (days - 1)); return { date, key: progressDayKey(date), label: new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: 'short' }).format(date).replace('.', '') }; }); }
   ...
   function progressInRange(value, start, end) { const date = new Date(value || 0); return Number.isFinite(date.getTime()) && date >= start && date < end; }
   ...
   function progressWeeklyValues() { const { start, end } = progressWeekRange(); const tests = progressCompletedTests().filter(item => progressInRange(item.date || item.finishedAt || item.completedAt, start, end)); const answers = progressAnswerRecords().filter(item => progressInRange(item.date || item.createdAt || item.created_at, start, end) && (item.correct === true || item.correct === false)); const reviewed = answers.length || tests.reduce((sum, item) => sum + Number(item.total || 0), 0); const mastered = answers.filter(item => String(item.statusAfter || '').toLowerCase().includes('master') && !String(item.statusBefore || '').toLowerCase().includes('master')).length; return { review: reviewed, tests: tests.length, mastered }; }
    ...
    function progressPeriodSummary() { const { start, end } = progressWeekRange(); const tests = progressCompletedTests().filter(item => progressInRange(item.date || item.finishedAt || item.completedAt, start, end)); const answers = progressAnswerRecords().filter(item => progressInRange(item.date || item.createdAt || item.created_at, start, end) && (item.correct === true || item.correct === false)); const masteredFromAttempts = answers.filter(item => String(item.statusAfter || item.status_after || '').toLowerCase().includes('master') && !String(item.statusBefore || item.status_before || '').toLowerCase().includes('master')).length; const masteredFromCards = allCards().filter(({ card }) => isMastered(card) && progressInRange(card.updatedAt || card.updated_at || card.masteredAt || card.mastered_at, start, end)).length; return { tests: tests.length, answers: answers.length || tests.reduce((sum, item) => sum + Number(item.total || 0), 0), mastered: masteredFromAttempts || masteredFromCards, reviews: progressSessionRecords().filter(item => progressInRange(item.finished_at || item.finishedAt || item.date || item.started_at || item.startedAt, start, end)).length || tests.length }; }
   ...
   function progressRecentStats() { const tests = progressCompletedTests().slice(0, 5); const total = tests.reduce((sum, item) => sum + Number(item.total || 0), 0); const score = tests.reduce((sum, item) => sum + Number(item.score || 0), 0); const times = progressAttemptsForTests(tests).map(progressResponseTime).filter(value => value >= PROGRESS_MIN_RESPONSE_MS); const averageTime = times.length ? times.reduce((sum, value) => sum + value, 0) / times.length : 0; const latest = tests[0]; return { tests, score, total, accuracy: total ? Math.round(score / total * 100) : null, averageTime, bestStreak: progressBestStreak(tests), latest: latest ? `${Number(latest.score || 0)} de ${Number(latest.total || 0)}` : 'Sem dados' }; }
    function progressDailyPoints(referenceDate = new Date()) { const days = progressDateRangeFrom(referenceDate, 7); const map = new Map(); const answerRows = progressAnswerRecords().filter(item => item.correct === true || item.correct === false); const recordedTests = new Set(answerRows.map(item => item.testId || item.test_id).filter(value => value != null).map(String)); const add = (date, correct, total) => { const key = progressDayKey(date); if (!key || !Number.isFinite(Number(total)) || Number(total) <= 0) return; const current = map.get(key) || { score: 0, total: 0 }; map.set(key, { score: current.score + Number(correct || 0), total: current.total + Number(total || 0) }); }; answerRows.forEach(item => add(item.date || item.createdAt || item.created_at, item.correct === true ? 1 : 0, 1)); progressCompletedTests().forEach(item => { const id = progressTestId(item); if (!answerRows.length || (id && !recordedTests.has(id))) add(item.date || item.finishedAt || item.completedAt, Number(item.score || 0), Number(item.total || 0)); }); return days.map(day => { const value = map.get(day.key); return { label: day.label, value: value && value.total ? Math.round(value.score / value.total * 100) : null }; }); }
    function progressChartNote(points) { const currentValues = points.map(point => point.value).filter(value => value != null); const previousValues = progressDailyPoints(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).map(point => point.value).filter(value => value != null); if (currentValues.length >= 1 && previousValues.length >= 1) { const currentAverage = currentValues.reduce((sum, value) => sum + value, 0) / currentValues.length; const previousAverage = previousValues.reduce((sum, value) => sum + value, 0) / previousValues.length; if (currentAverage > previousAverage) return 'Voc&ecirc; est&aacute; acima da sua m&eacute;dia anterior. Continue assim!'; if (currentAverage < previousAverage) return 'Seu aproveitamento caiu um pouco. Priorize as revis&otilde;es recomendadas.'; } return 'Continue estudando para acompanhar sua evolu&ccedil;&atilde;o.'; }
